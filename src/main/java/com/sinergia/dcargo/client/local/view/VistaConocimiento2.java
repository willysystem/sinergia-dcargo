...
@Singleton
public class VistaConocimiento2 extends View<Conocimiento> implements PresentadorConocimiento.Display {

	@Inject private VistaConocimientoAccion vistaConocimientoAccion;
	...
	private SuggestBox cajaBusquedaSuggestBox = new SuggestBox(transportistaOracle);
	private Widget     cajaBusquedaWidget;
	private ListBox usuariosListBox = new ListBox();
...
//	private HTML destinoLabel = new HTML("<b>Destino: </b>");
	...
	public VistaConocimiento2() {
		super(10);
	}
	
	protected VistaConocimiento2(int paging) {
		super(paging);
	}
	...
	public IsWidget viewIU(boolean esDialogBox) {
		...
		comboBusquedaListBox.addItem("Número de Conocimiento", "0");
		comboBusquedaListBox.addItem("Origen-Destino", "1");
		comboBusquedaListBox.addItem("Nombre de Transportista", "2");
		comboBusquedaListBox.addItem("Nombre de Usuario que elaboro el conocimiento", "3");
		comboBusquedaListBox.addItem("Borradores", "4");
		cajaBusquedaWidget.setWidth("250px");
		
		usuariosListBox.clear();
		log.info("  adminParametros.getUsuarios(): " + adminParametros.getUsuarios());
		for(Usuario usuario : adminParametros.getUsuarios()) 
			usuariosListBox.addItem(usuario.getNombreUsuario(), usuario.getId()+"");
		
		
		// Título
		HorizontalPanel hpTitulo = new HorizontalPanel();
		hpTitulo.setWidth("100%");
		hpTitulo.add(new HTML("<center class='tituloModulo'>Búsqueda de Conocimiento</center>"));
...
//	    vpNorte.add(layout);
		...
		horizontalPanelButtonSelect.add(inicioSelecionBtn);
...
//		else origenSuggestBox.setEnabled(false);
	...
	public Conocimiento getParametrosBusqueda() {
		...
		if((comboBusquedaListBox.getSelectedValue().equals("0")) &&  
			cajaBusquedaTextBox.getValue().equals("")) {
			mensajeAviso.mostrar("Parámetro numérico vacio");
			return null;
		}
		
		Integer nroConocimiento = null;
		if(comboBusquedaListBox.getSelectedValue().equals("0")) {
			String value = cajaBusquedaTextBox.getValue();
			try {
				nroConocimiento = Integer.parseInt(value);
		...
		if(comboBusquedaListBox.getSelectedValue().equals("1")) {
			...
			if((origenSuggestBox.getValue().equals("") && destinoSuggestBox.getValue().equals(""))) {
			...
			origen = adminParametros.buscarOficinaPorNombre(origenNombre);
			
			String destinoNombre = destinoSuggestBox.getValue();
			destino = adminParametros.buscarOficinaPorNombre(destinoNombre);
			
			fechaDesde = fechaIniDateBox.getValue();
			fechaHasta = fechaFinDateBox.getValue();
		}
		
		Transportista transportista = null;
		if(comboBusquedaListBox.getSelectedValue().equals("2")) {
			String transportistaNombre = cajaBusquedaSuggestBox.getValue();
			transportista = adminParametros.buscarTransportistaPorNombre(transportistaNombre);
			if(transportista == null) {
				mensajeAviso.mostrar("El transportista no fue elegido");
				return null;
			}
		}
		Usuario usuario = null;
		if(comboBusquedaListBox.getSelectedValue().equals("3")) {
			String nombreUsuario = usuariosListBox.getSelectedItemText();
			log.info("  nombreUsuario: " + nombreUsuario);
			usuario = adminParametros.buscarUsuarioPorNombreUsuario(nombreUsuario);
			if(usuario == null) {
				mensajeAviso.mostrar("Debe elegir un usuario");
				return null;
			}
		}
		...
		conocimiento.setFechaIni(fechaDesde);
		conocimiento.setFechaFin(fechaHasta);
...
//		conocimiento.setEstadoDescripcion(estadoListBox.getSelectedValue());
	...
	private void implementarAcciones() {
		...
		comboBusquedaListBox.addChangeHandler(e -> {
			...
			if(comboBusquedaListBox.getSelectedValue().equals("0")) {   
			...
			}  else if (comboBusquedaListBox.getSelectedValue().equals("1")) {
				cajaBusquedaTextBox.setEnabled(false);
				cajaBusquedaSuggestBox.setEnabled(false);
				habilitarOpcionesBusqueda(true);
			} else if (comboBusquedaListBox.getSelectedValue().equals("2")) {
				...
				cajaBusquedaWidget = usuariosListBox;
				layoutMain.setWidget(0, 2, cajaBusquedaWidget);
				cajaBusquedaSuggestBox.setEnabled(true);
				habilitarOpcionesBusqueda(false);
				//sinConocimientoCheckBox.setEnabled(true);
			} else if (comboBusquedaListBox.getSelectedValue().equals("4")) {
				habilitarOpcionesBusqueda(false);
				cajaBusquedaWidget = cajaBusquedaTextBox;
				layoutMain.setWidget(0, 2, cajaBusquedaWidget);
				cajaBusquedaTextBox.setEnabled(false);
		...
		consultarBtn.addClickHandler(e -> {
			...
			if(conocimiento == null)
				...
				vistaConocimientoAccion.setIsDialog(true);
		...
		anularBtn.addClickHandler(e -> {
			...
			if(conocimiento == null){
				...
				VistaConocimiento2.this.cargador.center();
				servicioConocimiento.cambiarEstado(conocimiento.getId(), "Anulado", new LlamadaRemota<Void>("No se pudo anular la Conocimiento", true) {
					...
					public void onSuccess(Method method, Void response) {
						...
						mensajeExito.center();
						VistaConocimiento2.this.cargador.hide();
					}
				});
			}
		});
		...
		inicioSelecionBtn.addClickHandler(e -> {
